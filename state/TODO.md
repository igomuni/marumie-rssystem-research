...
- **Next action (not yet started):** create and freeze case-003 Ground Truth for this row through direct visual source inspection, still before any benchmark engine is run against it.
- Not done, by design: no `ground-truth.json` (amounts/delta/normalized names not transcribed), no benchmark engine run against MIC, no Case Package (`document-profile.json`/`research-history.jsonl`) created for case-003 yet.
