...
- **Next action (not yet started):** apply the frozen protocol via direct visual inspection (no compared engine) to enumerate eligible rows within pages 5–245 and freeze exactly one target row.
- Not done, by design: no candidate row enumerated or inspected, no target row selected, no `ground-truth.json`, no benchmark engine run against MIC, no Case Package (`document-profile.json`/`research-history.jsonl`) created for case-003 yet.
